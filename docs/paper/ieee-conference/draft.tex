...
Four extensions follow directly from the limitations above. First, a live Wazuh deployment
...
strong GUIDE-specific results hold on a structurally different, SME-focused dataset. Third, the
RF-fallback reliability gap on non-GUIDE alert sources (Section~\ref{sec:discussion}) motivates
either labeled data collection on Wazuh-origin alerts specifically, or an origin-aware routing
change that defaults non-GUIDE alerts to human review rather than the RF path until validated.
Fourth, the full-graph red-team run (Section~\ref{sec:redteam}) needs a synthesized alert that
carries at least one of the context fields \texttt{route\_by\_context} checks, so the attack
payload actually reaches \texttt{classify\_with\_llm} instead of being diverted to the RF path
before the guardrails' target is ever tested; the remaining unresolved prompt-injection/roleplay
judge-model errors are a second, independent candidate for a stronger or differently-hosted judge
model if one becomes available.
