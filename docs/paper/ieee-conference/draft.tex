...
\author{
\IEEEauthorblockN{Asma\IEEEauthorrefmark{1}}
\IEEEauthorblockA{
\IEEEauthorrefmark{1}TECIP, Scuola Superiore Sant'Anna \\
CNIT/PNTLab Pisa \\
Pisa, Italy \\
% TODO: email/ORCID intentionally left blank -- per project issue #16,
% authorship, ORCID, and affiliation details are a supervisor-level
% decision deferred until closer to submission.
}
...
largest sample we ran (macro F1 0.669 vs. 0.751, $n=999$), a result we discuss rather than omit.
\end{abstract}
...
design examined in this paper. CORTEX \cite{cortex2025} argues for multi-agent, evidence-grounded
...
indistinguishable (mean 0.791 vs.\ 0.776); the best achievable accuracy across a threshold sweep
...
We use the Microsoft GUIDE dataset (CDLA-2.0), a public release of 1.6M real SOC alerts and 1M
...
Published LLM-based SOC alert-triage evaluations typically operate on bounded subsamples rather
than a full dataset, reflecting per-call inference cost and rate limits rather than a
methodological preference for small samples. Freitas et al., who introduced GUIDE itself, restrict
their evaluation of the investigation module to 1{,}000 randomly selected incidents out of GUIDE's
roughly one million, despite classical statistical baselines in the same paper being evaluated on
full regional splits an order of magnitude larger \cite{freitas2024guide}. Zhao et al.\ evaluate
LLM-based alert triage on stratified test splits of roughly 785 and 20{,}000 alerts drawn from two
production-derived corpora \cite{zhao2025infodense}. Our own evaluation samples of 300 and up to
999 alerts (Section~\ref{sec:hybrideval}) are consistent in order of magnitude with these
LLM-specific evaluation scales -- notably close to the 1{,}000-incident evaluation used by GUIDE's
own authors -- while remaining below the larger, tens-of-thousands-scale subsamples some papers
draw from smaller base corpora. We report this comparison for calibration, not as a claim that a
...
accuracy was 0.674 and macro F1 0.669 (79.1\% RF-routed, 20.9\% LLM-routed), a roughly 8-point
macro F1 gap below the RF baseline alone. We report both rather than the more favorable of the
two: the smaller sample was the one originally written up, and the larger, more statistically
robust sample was sitting in the repository's committed results without ever having been
discussed. We do not read this as a failure of the hybrid design so much as a reminder that raw
...
Splitting the 999-alert run by routing path isolates the source of the gap above: the RF-routed
subset (790 alerts, 79.1\%) alone scores macro F1 0.752, matching the standalone RF baseline almost
exactly, while the LLM-routed subset (209 alerts, 20.9\%) scores macro F1 0.308 -- barely above
chance for a three-class problem. This is the entire gap; RF's own accuracy is not the problem. It
