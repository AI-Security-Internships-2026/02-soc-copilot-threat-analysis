...
Security Operations Centers face alert volumes that outpace human triage capacity — a well-documented driver of analyst alert fatigue [20] — driving interest in LLM copilots that summarize, contextualize, and pre-classify alerts before an analyst sees them (Microsoft Security Copilot [1]; Ferrag et al.'s cybersecurity-LLM review [3]; Srinivas et al. [5]). A recurring design pattern in this space layers a fast, cheap guardrail (regex/keyword filtering) in front of a slower, more capable classifier, which in turn gates access to the LLM itself — mirroring the layered defense pattern used in general-purpose LLM guardrail frameworks such as Meta's LlamaFirewall [6] and Llama Prompt Guard [7], and Protect AI's LLM Guard [8].
...
**LLM-based SOC copilots.** Microsoft Security Copilot [1] demonstrates GPT-4 integrated into commercial SOC workflows, reporting reduced analyst triage time in a closed, proprietary system. Ferrag et al.'s review [3] surveys generative-AI/LLM applications across cybersecurity domains — including intrusion detection, threat intelligence, and malware detection — situating SOC-alert classification as one of many emerging LLM use cases rather than evaluating a single deployed pipeline against production data, which is the gap this paper's single-pipeline case study addresses. Wei et al.'s CORTEX [4] is the closest published architecture to ours: a multi-agent, evidence-grounded triage design that reduces false positives relative to single-model baselines, validating our own evidence-grounded (MITRE-context-fetching) LLM node design. Srinivas et al.'s survey [5] situates human-in-the-loop copilots as the dominant successful pattern in the field, over full automation — consistent with our pipeline's human-review checkpoint for non-high-confidence verdicts. The GUIDE dataset itself originates from Microsoft's own guided-response research [9], which we use directly as our evaluation corpus rather than a synthetic or network-flow substitute like CICIDS2017.
...
**MITRE ATT&CK-grounded LLM systems.** Recent work grounds LLM threat-intelligence reasoning in MITRE ATT&CK via retrieval-augmented generation — e.g., structured retrieval over ATT&CK technique descriptions to reduce hallucination in attack-technique attribution [10], knowledge-graph-augmented retrieval over cybersecurity knowledge bases [11], benchmarks for retrieval-augmented LLMs over heterogeneous cyber threat intelligence [12], and multi-label classification models for tagging free text against ATT&CK technique taxonomies [18] — a text-tagging task with the same reliance on a free-text input field that Section III.3 shows `AlertTitle` does not have. Freitas et al., who also released the GUIDE dataset [9] this paper evaluates on, separately report an adaptive incident-prioritization system for SOC alert queues at scale [19], underscoring that alert-volume triage remains an active research target beyond this paper's guardrail-specific scope. Our `fetch_mitre_context` pipeline stage (Section III) follows the same retrieval-before-classification pattern established by Lewis et al.'s foundational RAG formulation [13], applied to MITRE technique context rather than open-domain text.
...
*(ASCII rendering kept here for the Markdown draft; the submission version has a proper vector diagram — see `docs/paper/latex/ijis-draft.tex` §III.1, Figure 1, a TikZ diagram transcribed node-for-node and edge-for-edge from the compiled `StateGraph` in `src/agent/graph.py`, including the asymmetry that `rf_fallback` routes to `END`/`human_review` directly rather than through `parse_verdict`. `PROGRESS.md` item T2a.)*
...
### 3.2 Data
GUIDE [9, 22]: `datasets/GUIDE_train.csv` / `GUIDE_Test.csv`, Microsoft's public real-world SOC alert/incident corpus, evaluated on real triage-labeled alerts throughout (not synthetic substitutes). `AlertTitle`, the field targeted by both guardrail designs in this paper, contains 86,149 unique values in the training split, all integers — confirmed by direct inspection, not inferred from the schema documentation alone.

### 3.3 Guardrail decision process — why the classifier was rejected, not deployed

**Step 1 — issue-driven design.** Issue #10 requested a second-stage ML classifier behind the regex fast-path, citing prior benchmark work comparing Meta Llama Prompt Guard and Protect AI LLM Guard [6]–[8]. We pulled that benchmark data and additionally evaluated a TF-IDF + Logistic Regression detector (scikit-learn [21]), which won on both axes that matter for a gate placed in front of every alert: F1 0.883 vs. 0.747 (Prompt Guard) and 0.722 (LLM Guard); median latency 0.79ms vs. 179ms and 183ms — roughly three orders of magnitude faster (`experiments/results/` benchmark data referenced in PR #12).
...
![Per-alert classifier scores split by true label — the two classes overlap almost completely across the full score range.](figures/score_distribution.png)

*Figure: per-alert scores from `soc_domain_eval_results.json` (all 40 rows), generated by `docs/paper/figures/generate_figures.py` directly from the committed JSON — visual confirmation that the threshold sweep above fails because there is no threshold to find, not because the sweep stopped short (`PROGRESS.md` item T2b).*
...
**On the larger `agent_metrics_real_v3.json` (n=300, accuracy 0.28, macro F1 0.1836) reported elsewhere in the repository history:** this number is not a discrepancy to reconcile against the figure above — it is from a structurally different, earlier pipeline and is not comparable. Tracing `experiments/results/agent_metrics_real_v3.json` through `git log --diff-filter=A` shows it was generated in Week 5 (commit `d42e41a`/PR #5), before the sparse-alert RF-fallback route existed (added Week 6, PR #8) and before the schema guardrail and graph-wiring fix described in Sections III.3–III.4 (Week 8–9, PRs #12/#14/#15/#17) — i.e., against an agent with no fallback for sparse alerts and none of this paper's contributions. Its embedded "RF baseline: acc=0.374, f1=0.296" comparison is not a bug either: `git show d42e41a:experiments/results/baseline_metrics.json` confirms the baseline model at that point was trained on `n_train=6,400` rows and genuinely scored 0.374/0.296 — it was retrained on the full split (`n_train=79,580`, scoring 0.7718/0.7505, matching Section 4.4) only in the Week 9 fix (`ad02c85`). The comparison number baked into `real_v3.json` was accurate for the pipeline and baseline that existed when it was generated; it simply predates almost every contribution this paper describes. `agent_metrics_post_graph_fix_week9.json` (n=30) is therefore the only evaluation run against both the current pipeline and the current baseline, and is the number used in this paper — its small sample size is retained as an honest limitation (Section V) rather than papered over by citing a larger but non-comparable number. A fresh, larger post-fix run was not performed for this paper: it requires a live `GROQ_API_KEY` and the (gitignored) `baseline_model.joblib` artifact, neither available in the environment this section was written in.
...
![Throughput vs. worker count, faceted by pipeline mode (each panel has its own log-scale y-axis).](figures/throughput_scaling.png)

*Figure: throughput vs. worker count by mode, generated by `docs/paper/figures/generate_figures.py` directly from `week7_scalability_benchmark.json`. RF-only throughput scales with worker count as expected for local CPU-bound inference; LLM-only and hybrid do not, because Groq's per-minute token budget is shared across worker threads regardless of concurrency (`PROGRESS.md` item T2c).*
...
*(Numbered as cited above. Entries 1–5 from `docs/literature-review.md`; 6–20 newly sourced for this paper via web search. Author-list verification against publisher/arXiv/Crossref metadata (`PROGRESS.md` item R2) is complete for all entries, including #3 — see note below. Entries 18–22 were added to reach the ≥20-reference minimum, `PROGRESS.md` item R1.)*
...
3. Ferrag, Mohamed Amine; Alwahedi, Fatima; Battah, Ammar; Cherif, Bilel; Mechri, Abdechakour; Tihanyi, Norbert; Bisztray, Tamas; Debbah, Merouane. *Generative AI in Cybersecurity: A Comprehensive Review of LLM Applications and Vulnerabilities*. arXiv:2405.12750, 2024. (Replaces a fabricated entry from an earlier session — see note below.)
4. Wei, Bowen; Tay, Yuan Shen; Liu, Howard; Pan, Jinhao; Luo, Kun; Zhu, Ziwei; Jordan, Chris. *CORTEX: Collaborative LLM Agents for High-Stakes Alert Triage*. arXiv:2510.00311, 2025.
5. Srinivas, Siddhant; Kirk, Brandon; Zendejas, Julissa; Espino, Michael; Boskovich, Matthew; Bari, Abdul; Dajani, Khalil; Alzahrani, Nabeel. *AI-Augmented SOC: A Survey of LLMs and Agents for Security Automation*. Journal of Cybersecurity and Privacy (MDPI), 5(4), Art. 95, 2025. DOI: 10.3390/jcp5040095. (Journal corrected from "Informatics" — that was wrong; Crossref confirms this DOI belongs to the Journal of Cybersecurity and Privacy.)
6. Chennabasappa, Sahana; Nikolaidis, Cyrus; Song, Daniel; Molnar, David; Ding, Stephanie; Wan, Shengye; Whitman, Spencer; Deason, Lauren; Doucette, Nicholas; Montilla, Abraham; Gampa, Alekhya; de Paola, Beto; Gabi, Dominik; Crnkovich, James; Testud, Jean-Christophe; He, Kat; Chaturvedi, Rashnil; Zhou, Wu; Saxe, Joshua. *LlamaFirewall: An Open Source Guardrail System for Building Secure AI Agents*. arXiv:2505.03574, 2025.
...
9. Freitas, Scott; Kalajdjieski, Jovan; Gharib, Amir; McCann, Robert. *AI-Driven Guided Response for Security Operation Centers with Microsoft Copilot for Security* (GUIDE dataset). arXiv:2407.09017, 2024.
10. Alhuzali, Abeer. *LLM-powered threat intelligence: a retrieval-augmented generation approach for cyber attack investigation*. PeerJ Computer Science, 11, e3371, 2025. DOI: 10.7717/peerj-cs.3371. https://peerj.com/articles/cs-3371/
11. Yang, Lirong; Zhu, Hongliang; Mu, Jin; Li, Qi. *KGAgent4CTI: unlocking the power of LLM in threat intelligence*. Cybersecurity (Springer Nature), 9(1), Art. 76, 2026. DOI: 10.1186/s42400-025-00458-2. (Year corrected to 2026 per Crossref's published-online date; the "-025-" in the DOI reflects manuscript intake, not the publication year.) https://link.springer.com/article/10.1186/s42400-025-00458-2
12. Cheng, Yutong; Liu, Yang; Li, Changze; Song, Dawn; Gao, Peng. *CTIConnect: A Benchmark for Retrieval-Augmented LLMs over Heterogeneous Cyber Threat Intelligence*. arXiv:2510.11974, 2025.
...
14. Wang, Peiran; Li, Xinfeng; Xiang, Chong; Zhang, Jinghuai; Li, Ying; Zhang, Lixia; Wang, Xiaofeng; Tian, Yuan. *The Landscape of Prompt Injection Threats in LLM Agents: From Taxonomy to Analysis*. arXiv:2602.10453, 2026.
15. Chowdhury, Arijit Ghosh; Islam, Md Mofijul; Kumar, Vaibhav; Shezan, Faysal Hossain; Kumar, Vaibhav; Jain, Vinija; Chadha, Aman. *Breaking Down the Defenses: A Comparative Survey of Attacks on Large Language Models*. arXiv:2403.04786, 2024. ("Vaibhav Kumar" appears twice in the byline on the arXiv page itself, not a transcription duplicate.)
16. Das, Badhan Chandra; Amini, M. Hadi; Wu, Yanzhao. *Security and Privacy Challenges of Large Language Models: A Survey*. arXiv:2402.00888, 2024.
...
18. Crossman, Andrew; Dodd, Jonah; Kumar, Viralam Ramamurthy Chaithanya; Mohammed, Riyaz; Plummer, Andrew R.; Sekharudu, Chandra; Warrier, Deepak; Yekrangian, Mohammad. *Constructing Multi-label Hierarchical Classification Models for MITRE ATT&CK Text Tagging*. arXiv:2601.14556, 2026.
19. Freitas, Scott; Gharib, Amir; Magenheim, Maayan. *Adaptive Incident Prioritization for Security Operations at Scale*. arXiv:2607.16963, 2026.
20. Ndichu, Samuel; Ban, Tao; Ozawa, Seiichi; Takahashi, Takeshi; Inoue, Daisuke. *AI-Driven Security Alert Screening and Alert Fatigue Mitigation in Security Operations Centers: A Survey*. arXiv:2605.08316, 2026. (Title on the arXiv page itself reads "...: A Survey", not "...: A Comprehensive Survey" as originally tracked in `PROGRESS.md`.)
21. Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion, B., Grisel, O., Blondel, M., Prettenhofer, P., Weiss, R., Dubourg, V., Vanderplas, J., Passos, A., Cournapeau, D., Brucher, M., Perrot, M., Duchesnay, E. *Scikit-learn: Machine Learning in Python*. Journal of Machine Learning Research, 12, pp. 2825–2830, 2011.
22. Microsoft. *Microsoft Security Incident Prediction* (GUIDE dataset, Kaggle release). Kaggle, 2024. https://www.kaggle.com/datasets/Microsoft/microsoft-security-incident-prediction

**Resolved fabrication (entry 3):** the original entry 3 (`ferrag2024`, claimed as "Automated Cyber Threat Intelligence: LLMs for SIEM Alert Triage", arXiv:2407.08628) was fabricated in an earlier session — that arXiv ID resolves to an unrelated math.AP paper, and no real paper under that title exists. Replaced with the closest genuine Ferrag et al. LLM-security paper, confirmed via direct fetch of its arXiv abstract page: a broad review of generative-AI/LLM applications across cybersecurity domains (intrusion detection, threat intelligence, malware detection, among others), **not** a dedicated SIEM-alert-triage or CICIDS2017 evaluation. The two places this citation is used in Sections I and II were rewritten to describe what this paper actually covers rather than the fabricated claim — see the corrected text there and the matching comment in `references.bib`.

---

## Appendix (not yet started)
The full scalability benchmark table (§4.6) and full per-alert results tables are now in the body, not pending. Still not started: raw `EXPECTED_NUMERIC_FIELDS` validation script output and per-run CPU/memory fields (process CPU seconds, peak RSS) from `week7_scalability_benchmark.json`, omitted from §4.6 as not needed to support this paper's claims.
