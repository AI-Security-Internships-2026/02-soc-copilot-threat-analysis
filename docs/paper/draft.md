...
- Real data (5,000 `AlertTitle` values sampled from `GUIDE_train.csv`): **not yet re-run in this checkout** — `datasets/GUIDE_train.csv` is gitignored and wasn't present when this draft was generated. `experiments/results/schema_guardrail_eval.json` currently records `"status": "skipped"` for this section. The "0 false positives on 5,000 real values" figure still traces only to PR #15's description until someone with the dataset locally reruns `experiments/schema_guardrail_eval.py` and commits the populated result. **Do not cite the 5,000-row number in the final paper until that file shows `"status": "ran"`.**
...
- The schema guardrail's real-data validation (Section 4.3) currently exists only as PR-description prose and a module docstring, not a committed, reproducible results artifact — flagged as `PROGRESS.md` item E1.
