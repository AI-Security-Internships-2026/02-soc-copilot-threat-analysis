...
Source: \path{experiments/results/week7_scalability_benchmark.json}.

\begin{itemize}
\item Regex guardrail microbenchmark: mean check time 3.616\,\textmu s/alert over 10{,}000 iterations; 10{,}000/10{,}000 injection alerts blocked, 0/10{,}000 benign alerts blocked (at that stage's test set).
\item LLM path (1 worker, 30 prompts): throughput 0.5672 alerts/s, mean latency 1.7629\,s --- dominated by remote Groq inference and network latency, not local compute.
\end{itemize}

\noindent\emph{Full table --- CPU/memory/worker-scaling rows --- to be transcribed in full for the final version; only the headline rows are drafted here. See \texttt{PROGRESS.md} item E3.}
