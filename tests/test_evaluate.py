...
These exist because of a real review finding on PR #21: an earlier "fix" made
crash diagnostics better (triage_path flips to "error", the real exception
message shows up in reasoning) but left the actual bug in place underneath --
a crashed row still silently became predicted="FalsePositive" and fed straight
into y_pred/metrics, exactly like a real (successful) FalsePositive verdict
would. If the whole graph broke again, accuracy/macro-F1 would look plausible
instead of visibly wrong, and the only way to catch it would be manually
diffing routing_summary counts against sample_size. Nothing here should be
able to regress that silently again.
"""
...
def test_all_rows_crashing_raises_instead_of_reporting_metrics_from_nothing(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="all 2 rows crashed"):
