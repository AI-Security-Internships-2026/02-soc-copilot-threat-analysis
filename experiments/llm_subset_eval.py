...
def load_llm_eligible_sample(n_per_class: int | None, seed: int = 42) -> pd.DataFrame:
    """LLM-routed alerts out of the 999-alert cache, keeping their cache position.

    n_per_class=None takes every eligible alert (209 of them), which is what
    produced llm_subset_eval_improved_full209.json -- the file behind the
    0.2823 headline, the McNemar p-value and the calibration inversion.

    The position of each alert in the cache is carried through as _row_index.
    Without it there is no way to line these results up against the Random
    Forest's predictions for the same alerts, which is what makes the
    comparison paired; experiments/rf_vs_llm_control.py requires the field and
    refuses to run without it. This function used to end in
    .reset_index(drop=True), which threw that mapping away, so the committed
    full-209 artifact could not be regenerated by any script in the repository.
    """
    df = pd.read_csv(CACHE_PATH)
    df = df[df.apply(lambda r: not should_use_fallback(r.to_dict()), axis=1)]
    if n_per_class is not None:
        df = df.groupby("IncidentGrade", group_keys=False).apply(
            lambda g: g.sample(n=min(n_per_class, len(g)), random_state=seed)
        )
    return df.reset_index().rename(columns={"index": "_row_index"})


def run(prompt_variant: str, n_per_class: int | None, output_path: str):
    system_prompt = PROMPTS[prompt_variant]
    sample = load_llm_eligible_sample(n_per_class)
    scope = "every eligible alert" if n_per_class is None else f"{n_per_class}/class"
    print(f"running '{prompt_variant}' prompt on {len(sample)} LLM-eligible alerts "
          f"({scope}, evidence_field_count >= 2)...")

    y_true, y_pred, log = [], [], []
    for i, row in sample.iterrows():
        alert = row.to_dict()
        row_index = int(alert.pop("_row_index"))
        ...
        log.append({"_row_index": row_index, "ground_truth": ground_truth, **result})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all-eligible",
        action="store_true",
        help="score every LLM-routed alert in the cache (209) instead of "
             "sampling n-per-class. This is the setting that produced "
             "llm_subset_eval_improved_full209.json.",
    )
    parser.add_argument("--output", type=str, default=None)
    args = parser.parse_args()
    output_path = args.output or f"experiments/results/llm_subset_eval_{args.prompt}.json"
    run(args.prompt, None if args.all_eligible else args.n_per_class, output_path)
